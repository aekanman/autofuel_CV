...
int GeneratePositionMessage(int x, int y, int z, char buffer[10]) //out?
{
	...
	buffer[3] = (char)((x >= 0) ? 0x00 : 0x01);							// first byte represents the sign; 0: positive, 1: negative
	buffer[4] = (char)abs(x);											// absolute of the int value
	buffer[5] = (char)((y >= 0) ? 0x00 : 0x01);
	buffer[6] = (char)abs(y);
	buffer[7] = (char)((z >= 0) ? 0x00 : 0x01);
	buffer[8] = (char)abs(z);
